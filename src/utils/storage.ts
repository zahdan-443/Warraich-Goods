...
export function normalizeBiltyConfig(data: any): BiltyAccessConfigData {
  const allowedUIDs = Array.isArray(data?.allowedUIDs) ? data.allowedUIDs.filter(Boolean) : [];
  const rawEmails = Array.isArray(data?.allowedEmails) ? data.allowedEmails.filter(Boolean) : [];
  const allowedEmails = Array.from(new Set(rawEmails.map((e: string) => String(e).toLowerCase().trim())));
