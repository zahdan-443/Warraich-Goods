...
export const TollCalculatorModal: React.FC<TollCalculatorModalProps> = ({
  ...
  return (
    <div 
      className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 overflow-y-auto font-sans select-none"
      dir={isUrdu ? 'rtl' : 'ltr'}
      onClick={(e) => {
        if (e.target === e.currentTarget) onClose();
      }}
    >
      <div className="bg-[#fcfbf7] text-[#4a4a35] w-full max-w-2xl max-h-[95dvh] sm:max-h-[90dvh] flex flex-col rounded-3xl border border-[#ecece0] shadow-2xl overflow-hidden my-auto animate-in fade-in zoom-in-95 duration-150">
        
        {/* Top Sticky Header */}
        <header className="flex items-center justify-between bg-white px-3.5 sm:px-4 py-2.5 sm:py-3 border-b border-[#ecece0] shrink-0">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="w-9 h-9 rounded-xl bg-white border border-[#ecece0] p-1 flex items-center justify-center shrink-0 shadow-2xs overflow-hidden">
              <PublicImage
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 flex-wrap">
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold bg-[#8b9d77]/15 text-[#5a5a40]">
                  <Milestone className="w-2.5 h-2.5 text-[#8b9d77]" />
                  {isUrdu ? 'این ایچ اے 2026 ٹول' : 'NHA 2026 Toll'}
                </span>
                ...
                <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-semibold border ${
                  ...
                  {isOnline ? <Wifi className="w-2 h-2 text-emerald-600" /> : <WifiOff className="w-2 h-2 text-amber-600" />}
                  <span>{isOnline ? (isUrdu ? 'لائیو ریٹس' : 'Live') : (isUrdu ? 'محفوظ کیش' : 'Cached')}</span>
                </span>
                ...
                <button
                  ...
                  disabled={isSyncing}
                  className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-[#fdfbf7] hover:bg-[#8b9d77] hover:text-white text-[#5a5a40] border border-[#ecece0] transition-all cursor-pointer shadow-2xs"
                  title={isUrdu ? 'ریٹس ریفریش کریں' : 'Sync 2026 NHA Rates'}
                >
                  <RefreshCw className={`w-2 h-2 ${isSyncing ? 'animate-spin' : ''}`} />
                  <span>{isSyncing ? (isUrdu ? 'سنک...' : '...') : (isUrdu ? 'ریفریش' : 'Sync')}</span>
                </button>
              </div>

              <h1 className="text-xs sm:text-sm font-serif font-bold text-[#4a4a35] truncate mt-0.5">
                {isUrdu ? 'موٹروے و شاہراہ ٹول ٹیکس کیلکولیٹر' : t.tollCalc.title}
              </h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-2 bg-[#f0f0e4] hover:bg-[#5a5a40] hover:text-white rounded-xl text-[#5a5a40] transition-all cursor-pointer active:scale-95"
        ...
        {/* Mode Switcher Tabs */}
        <div className="px-3 pt-2.5 pb-1 shrink-0 bg-[#fcfbf7]">
        ...
        <main className="flex-1 overflow-y-auto px-3 sm:px-4 py-2.5 space-y-2.5">
            ...
            <>
              ...
              {viewState === 'input' && (
                <div className="space-y-2.5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-11 gap-2 items-center">
                      ...
                      <div className="sm:col-span-5 space-y-1">
                        ...
                        <select
                          ...
                          className="w-full bg-[#fdfbf7] border border-[#ecece0] rounded-xl px-2.5 py-1.5 sm:py-2 text-xs sm:text-sm font-medium text-[#4a4a35] focus:outline-none focus:border-[#8b9d77] shadow-2xs cursor-pointer"
                        >
                          {CITIES_LIST.map(c => (
                  ...
                  <button
                    ...
                    className="w-full py-3 rounded-2xl bg-gradient-to-r from-[#162a4d] to-[#1e3a68] hover:from-[#12223e] hover:to-[#162a4d] text-white font-serif font-bold text-xs sm:text-sm transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer active:scale-98 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Calculator className="w-4 h-4 text-amber-300" />
                    <span>{isUrdu ? 'موٹروے و شاہراہ ٹول ٹیکس کا تخمینہ لگائیں' : 'Calculate Toll Tax'}</span>
                  </button>
                </div>
              )}
              ...
              {viewState === 'result' && result && (
                <div className="space-y-2.5 animate-in fade-in duration-150">
                  ...
                  <div className="flex items-center justify-between bg-white p-2 sm:p-2.5 rounded-xl border border-[#ecece0] shadow-2xs">
                    <button
                      type="button"
          ...
          {activeTab === 'motorways' && (
            <div className="space-y-2.5 animate-in fade-in">
              ...
              <div className="bg-gradient-to-br from-[#162a4d] to-[#1e3a68] text-white p-3 sm:p-4 rounded-2xl shadow-md space-y-2.5">
                ...
                <div className="bg-black/20 p-2 rounded-xl space-y-0.5 max-h-20 overflow-y-auto">
        ...
        {/* Sticky Clean Bottom Action Footer */}
        <footer className="bg-white px-3 sm:px-4 py-2.5 border-t border-[#ecece0] flex items-center justify-between gap-2 shrink-0">
          {activeTab === 'route' && viewState === 'result' && result ? (
            <>
              <div className="flex items-center gap-1.5 flex-wrap">
                {onApplyToTrip && (
                  <button
                    ...
                    className="px-3 py-2 rounded-xl bg-[#8b9d77] hover:bg-[#798a67] text-white text-xs font-bold font-serif transition-all shadow-xs flex items-center gap-1.5 cursor-pointer active:scale-95"
                  >
                    <ArrowRight className="w-3.5 h-3.5" />
                    <span>{isUrdu ? 'حساب میں شامل کریں' : t.tollCalc.applyToTrip}</span>
                  </button>
                )}
                ...
                <button
                  type="button"
                  onClick={handleCopy}
                  className="px-2.5 py-2 rounded-xl bg-[#fdfbf7] border border-[#ecece0] hover:bg-[#f0f0e4] text-[#4a4a35] text-xs font-bold transition-all shadow-2xs flex items-center gap-1 cursor-pointer active:scale-95"
                ...
                <button
                  ...
                  className="px-2.5 py-2 rounded-xl bg-[#25D366] hover:bg-[#20bd5a] text-white text-xs font-bold transition-all shadow-xs flex items-center gap-1 cursor-pointer active:scale-95"
              ...
              <div className="flex items-center gap-1.5">
                <button
                  type="button"
                  onClick={() => setViewState('input')}
                  className="px-3 py-2 rounded-xl bg-[#f0f0e4] hover:bg-[#5a5a40] hover:text-white text-[#4a4a35] text-xs font-bold font-serif transition-all cursor-pointer active:scale-95"
                >
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="px-3 py-2 rounded-xl bg-[#4a4a35] hover:bg-[#383827] text-white text-xs font-bold font-serif transition-all cursor-pointer active:scale-95"
                >
            ...
            <>
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="px-2.5 py-2 rounded-xl bg-[#fdfbf7] border border-[#ecece0] text-[#162a4d] text-xs font-bold transition-all shadow-2xs flex items-center gap-1 cursor-pointer active:scale-95"
                >
                ...
                <button
                  ...
                  className="px-2.5 py-2 rounded-xl bg-[#25D366] text-white text-xs font-bold transition-all shadow-xs flex items-center gap-1 cursor-pointer active:scale-95"
                >
                  <Share2 className="w-3.5 h-3.5" />
                  <span>{isUrdu ? 'واٹس ایپ' : 'WhatsApp'}</span>
                </button>
                ...
                {onApplyToTrip && (
                  <button
                    ...
                    className="px-3 py-2 rounded-xl bg-amber-400 hover:bg-amber-300 text-[#162a4d] text-xs font-black transition-all shadow-xs flex items-center gap-1 cursor-pointer active:scale-95"
                  >
                    <ArrowRight className="w-3.5 h-3.5" />
              ...
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 rounded-xl bg-[#4a4a35] hover:bg-[#383827] text-white text-xs font-bold font-serif transition-all cursor-pointer active:scale-95"
              >
                {isUrdu ? 'بند کریں' : 'Close'}
              </button>
            </>
            ...
            <div className="flex items-center justify-between w-full">
              <span className="text-[10px] text-[#8e8e75] font-mono">
                {isUrdu ? 'پاکستان نیشنل ہائی وے اتھارٹی ریٹس 2026' : 'National Highway Authority Pakistan (2026)'}
              </span>
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 rounded-xl bg-[#f0f0e4] hover:bg-[#5a5a40] hover:text-white text-[#4a4a35] text-xs font-bold font-serif transition-all cursor-pointer active:scale-95"
              >
                {isUrdu ? 'بند کریں' : 'Close'}
              </button>
            </div>
